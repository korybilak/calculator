...
def equalpress():
    try:
        ...
        global expression
        expression = re.sub('√', 'math.sqrt', expression)
        if '^' in expression:
             sub_expression = re.findall('\d+\^\d+', expression)
             for subexp in sub_expression:
                subexp = subexp.split('^')
                inta = subexp[0]
                intb = subexp[1]
                expression = re.sub('\d+\^\d+', 'math.pow(' + inta + ',' + intb + ')', expression, 1)
...
if __name__ == "__main__":
    ...
    sqrt = Button(gui, text='√', fg='black', bg='white',
                    command=lambda: press('√('), height=1, width=7)
    sqrt.grid(row=6, column=1)
    
    openparen = Button(gui, text='(', fg='black', bg='white',
                    command=lambda: press('('), height=1, width=7)
    openparen.grid(row=6, column=2)
    
    closeparen = Button(gui, text=')', fg='black', bg='white',
                    command=lambda: press(')'), height=1, width=7)
    closeparen.grid(row=6, column=3)
    gui.mainloop()